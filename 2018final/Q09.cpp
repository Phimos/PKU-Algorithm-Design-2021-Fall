...
bool can_reach(int i, int j, double d) {
    return (double(x[i] - x[j]) * double(x[i] - x[j]) +
            double(y[i] - y[j]) * double(y[i] - y[j])) < d * d;
}
    ...
    while (test_cnt--) {
        ...
        for (int k = 1; k <= num; ++k) {
            ...
            for (int i = 1; i <= num; ++i) {
                ...
                add_edge(i, num + i, m[i], 0);
                for (int j = 1; j <= num; ++j) {
                    if (can_reach(i, j, d)) {
                        add_edge(num + i, j, INT_MAX, 0);
                    }
                }
            }
            add_edge(k, t, INT_MAX, 0);
            if (dinic(s, t) == cnt) {
                printf("%d ", k - 1);
                flag = true;
            }
